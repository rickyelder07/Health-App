...
class PhotoService {
    ...
    func uploadPhoto(
        ...
        do {
            ...
            print("🔑 Uploading to path: \(userId.uuidString.lowercased())/...")
        } catch {
        ...
        let filename = "\(userId.uuidString.lowercased())/\(timestamp)_full.jpg"
        ...
        if let thumbnailData = generateThumbnail(from: image, maxSize: 300) {
            let thumbFilename = "\(userId.uuidString.lowercased())/\(timestamp)_thumb.jpg"
    ...
    func fetchPhotos(
        ...
        let response = try await supabase
            ...
            .select()
            .eq("user_id", value: userId.uuidString)
            .gte("date_taken", value: startDate.ISO8601Format())
            .lte("date_taken", value: endDate.ISO8601Format())
            .order("date_taken", ascending: false)
            .execute()
