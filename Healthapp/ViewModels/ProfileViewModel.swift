...
class ProfileViewModel: ObservableObject {
    ...
    @Published var useMetric: Bool = UserSettings.load().weightUnit == .kg
    ...
    func loadCurrentProfile() {
        ...
        // Load values converted to current unit preference
        if let userWeight = user.weight {
            let display = useMetric ? userWeight : userWeight * 2.20462
            weight = String(format: "%.1f", display)
        }

        if let userHeight = user.height {
            let display = useMetric ? userHeight : userHeight / 2.54
            height = String(format: "%.0f", display)
        }
    ...
    func toggleUnits() {
        ...
        settings.weightUnit = useMetric ? .kg : .lbs
        settings.heightUnit = useMetric ? .cm : .feet
        settings.distanceUnit = useMetric ? .km : .miles
        settings.save()
    }
